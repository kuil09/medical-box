...
def home() -> HTMLResponse:
    return page(
        "가족 구급키트를 한눈에",
        "<p>공용 트레이와 개인 파우치를 정리하고, 보관약과 사용기한을 기기 안에서 관리합니다.</p>"
...
def privacy() -> HTMLResponse:
    return page(
        ...
        "<p>가족, 보유 의약품, 유효기간, 메모, 방문 일정과 알림 정보는 "
        "사용자의 암호화된 기기 데이터베이스에만 저장됩니다. 계정 서버로 "
        "동기화하지 않습니다.</p>"
        "<h2>필수 계정</h2>"
        "<p>앱 사용을 위해 가입 또는 로그인이 필요합니다. 로그인 제공자, "
        "제공자 계정 식별자, 표시 이름, "
        "이메일, 약관 동의 기록, 검색 권한과 로그인 세션 해시를 처리합니다. "
        "로그아웃하면 앱은 잠기지만 기기의 암호화된 구급키트 데이터는 삭제되지 않습니다.</p>"
        "<h2>국외 처리위탁·보관</h2>"
        ...
        "국외 이전을 원하지 않으면 계정을 만들지 않거나 기존 계정을 삭제할 수 있으며, "
        "계정이 없으면 앱을 사용할 수 없습니다.</p>"
        "<h2>검색과 로그</h2>"
        "<p>공식 의약품을 검색하거나 상세 정보를 열면 로그인 계정과 검색 권한을 "
        "확인하는 인증 요청에 검색어 또는 공개 품목기준코드(itemSeq)가 포함되어 "
        "서버에서 처리됩니다. 서버는 이를 유효기간, 메모 등 기기 안의 "
        "보관 정보와 결합하거나 영구 저장하지 않습니다.</p>"
        "<p>의약품 검색어, 요청 본문과 응답 본문은 애플리케이션 로그나 분석 "
        "시스템에 저장하지 않습니다.</p>"
        "<h2>광고</h2>"
        "<p>광고 기능은 현재 비활성 상태입니다. 실제 광고 사업자를 연결하기 전에 "
        "수집 항목, 처리위탁자, 국외 처리, 보유기간과 추적 여부를 이 방침과 앱스토어 "
        "표시에 추가하고 별도 검증합니다. 의약품명, 검색어, 가족·재고·메모·일정·알림, "
        "계정 식별자를 광고 타기팅에 사용하지 않습니다.</p>",
    )
...
def terms(settings: Settings = Depends(get_settings)) -> HTMLResponse:
    return page(
        ...
        "<p>앱 사용에는 계정 생성 또는 로그인이 필요합니다.</p>"
